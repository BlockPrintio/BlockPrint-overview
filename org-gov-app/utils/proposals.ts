...
export const PROPOSAL_OBJECTIVES: Record<string, string> = {
    ...
    '1200220': 'Maintenance and operations of Cardano SDK, community support and content creation, in order to onboard developers and users to the Cardano blockchain ecosystem',
    '1200147': 'Upgrade Cardano tools by implementing CIP 45, WebRTC wallet connect, handle multiple serialization libs, revamp to support backend transactions building, and improve error messages to improve DevXP',
    ...
    '1300130': 'Maintenance and operations of Cardano open source libraries and tool suits, maintaining discord dev support and core operations',
    ...
    '1300134': 'Enhancing Cardano Devtools by migrating features, developing Rust-based validation modules, and improving error handling to provide clearer feedback, helping developers debug and validate efficiently',
...
export async function getProposalDetails(projectId: string): Promise<ProposalDetails | null> {
    try {
        const response = await fetch(`https://gov.meshjs.dev/api/proposals/${projectId}`);
        if (!response.ok) {
