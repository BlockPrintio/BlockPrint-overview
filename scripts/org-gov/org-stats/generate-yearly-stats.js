...
async function loadPreviousStats(year) {
    ...
    const repoRoot = getRepoRoot();
    try {
        const statsPath = path.join(repoRoot, BASE_DIR, STATS_DIR, `org-yearly-stats-${year}.json`);
...
async function main() {
    ...
    try {
        console.log(`Starting Yearly ${config.mainOrganization.displayName} Stats Generation...\n`);
